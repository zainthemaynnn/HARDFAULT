...
local function ProcessNode(node, nodes)
	...
	elseif node.type == "externaltask" then
		assert(node.params.run, "Can't process tree; externaltask leaf node has no run func parameter")

		local taskNode = addNode("externaltask")
		taskNode.start = node.params.start
		taskNode.run = node.params.run
		taskNode.finish = node.params.finish
		taskNode.onsuccess = true
